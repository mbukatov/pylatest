...
class FooBar(rst.Directive):
    ...
    def run(self):
        # each foobar directive in the text has mandatory id (starts from 1),
        # so there is either just one or no foobar directive with
        # foobar_id == 1
        foobar_id = int(self.arguments[0])
        ...
        pending.details['foobar_id'] = foobar_id
        # since foobar transformation will process all foobar pending nodes
        # at once, we register pending node with foobar_id == 1 only,
        # this also means that the result generated by foobar transform
        # will be located in the place of foobar directive #1
        if foobar_id == 1:
